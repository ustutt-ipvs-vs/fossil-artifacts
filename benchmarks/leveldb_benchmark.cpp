...
private:
    ...
    fossil::reference<pm> reference_;
...
public:
    Benchmark()
        : num_(FLAGS_num),
          ...
          reference_(fossil::object_repo().create<pm>())
    {}
    ...
    void Run()
    {
        ...
        while(benchmarks != nullptr) {
            ...
            void (Benchmark::*method)(ThreadState*, fossil::reference<pm>) = nullptr;
            bool fresh_db = false;
...
private:
    struct ThreadArg
    {
        ...
        SharedState* shared;
        ThreadState* thread;
        fossil::reference<pm> reference;
        void (Benchmark::*method)(ThreadState*, fossil::reference<pm>);
    };
    ...
    static void ThreadBody(void* v)
    {
        ...
        thread->stats.Start();
        (arg->bm->*(arg->method))(thread, arg->reference);
        thread->stats.Stop();
    ...
    void WriteSeq(ThreadState* thread, fossil::reference<pm> ref) { DoWrite(thread, ref, true); }

    void WriteRandom(ThreadState* thread, fossil::reference<pm> ref)
    {
        DoWrite(thread, ref, false);
    }

    void DoWrite(ThreadState* thread, fossil::reference<pm> ref, bool seq)
    {
        RandomGenerator gen;
        int64_t bytes = 0;
    ...
    void ReadRandom(ThreadState* thread, fossil::reference<pm> ref)
    {
        std::string value;
        int found = 0;
