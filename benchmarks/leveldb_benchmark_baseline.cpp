    ...
    void Run()
    {
        ...
        while(benchmarks != nullptr) {
            ...
            void (Benchmark::*method)(ThreadState*,
                                      std::shared_mutex&,
                                      std::unordered_map<std::uint64_t, std::string>&) = nullptr;
            bool fresh_db = false;
...
private:
    ...
    static void ThreadBody(void* v)
    {
        ...
        thread->stats.Start();
        (arg->bm->*(arg->method))(thread, *arg->mtx, *arg->map);
        thread->stats.Stop();
                 ...
                 bool seq)
    {
        ...
        for(int i = 0; i < num_; ++i) {
            const auto k = seq ? i : thread->rand.Uniform(FLAGS_num);
                    ...
                    std::unordered_map<std::uint64_t, std::string>& map)
    {
        ...
        for(int i = 0; i < reads_; i++) {
            const auto k = thread->rand.Uniform(FLAGS_num);

            {
                std::shared_lock lock{mtx};
                if(auto iter = map.find(k); iter != std::end(map)) {
                    value = iter->second;
                    found++;
                }
            }
